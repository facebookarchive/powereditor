...
var AdvDem = view.newClass('ads.adEditor.AdvDem', Base, {
    ...
    _prepare: function(callback) {
        Completion.prepare(['locales'], callback);
    },
    ...
    _createDom: function(initArgs) {
        ...
        this.content({
            ...
            birthday: {
                view: 'Checkbox',
                text: 'Target people on their birthdays',
            ...
            interested: {
                view: controls.Sex,
                childName: 'interested_in',
                name: 'interested_in'
            },

            relationship_label: 'Relationship',
            relationship: {
                view: controls.Relationship,
            ...
            langs: {
                ...
                childName: 'langs',
                info2value: function(info) {
                  return info.id;
                },
                value2info: function(id) {
                  var data = Completion.cached('locales').searchById(id);
                  return data && { id: data.id, text: data.value };
                },
                data: (new LocalDataSource())
                    .maxResults(AdEditorConstants.MAX_RESULTS_DEFAULT)
                    .queryEndpoint({
                      search: Completion.createSearcher('locales')
                    })
            }
        });
