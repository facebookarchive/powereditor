...
function camplink(camp) {
  ...
  // adgroups.php?campaign_id=#&act=#
  if (!camp.isNew() && (!camp.camplink() || camp.isChangedSelf())) {
    var camp_link =
      '/ads/manage/adgroups.php?campaign_id=' + camp.id() +
      '&act=' + camp.account_id();
    var name = '<a class="adPane-link" target="_blank" href=' +
      camp_link + '>' + dom.escapeHTML(camp.id()) + '</a>';
    camp.camplink(name);
  }
  return camp.camplink() || camp.id();
}

exports.camplink       = camplink;
